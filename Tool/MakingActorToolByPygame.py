...
helpText = ['ctrl+P:Making Polygon','ctrl+C:Making Circle', 'ctrl+q:Finish draw shape','ctrl+F:Finish and extract.', 'ctrl+d:Delete Shape', 'ctrl+M:move Screen', 'Set mode and click Coordinate.', 'esc or alt+F4: Close']
...
def renderMultiLineText(screen, coordinate, textList, fontSize, color, backgroundColor, antialias=True):
    ...
    pygame.draw.rect(screen, backgroundColor, [coordinate[0], coordinate[1], round(maxTextLen*fontSize*0.5), (len(textList)+1)*fontSize])#background
...
def dotToScreenDot(dot, lineDistance, SCREEN_SIZE, screenCoordinate):
    screenDot = (screenCoordinate[0]//lineDistance, screenCoordinate[1]//lineDistance)
    return ((dot[0]-screenDot[0])*lineDistance, (SCREEN_SIZE[1]-(dot[1]-screenDot[1])*lineDistance))#+screenDot[1]*lineDistance)
...
while not done:
    ...
    for i in range(SCREEN_SIZE[1]//lineDistance):#draw x line, linevsline distance
        pygame.draw.line(screen, BLACK, [0,i*lineDistance], [SCREEN_SIZE[0], i*lineDistance], 1)
    
    for i in range(SCREEN_SIZE[0]//lineDistance):#draw y line, linevsline distance
    ...
    if mode == 'MakingPoly':
        for i in range(len(nowShape.dotList)-1):
            if len(nowShape.dotList)>1:
                pygame.draw.line(screen, BLACK, dotToScreenDot(nowShape.dotList[i], lineDistance, SCREEN_SIZE, screenCoordinate), dotToScreenDot(nowShape.dotList[i+1], lineDistance, SCREEN_SIZE, screenCoordinate))
            drawDot(screen, dotToScreenDot(nowShape.dotList[i], lineDistance, SCREEN_SIZE, screenCoordinate), BLACK)
        if len(nowShape.dotList)>0:
            drawDot(screen, dotToScreenDot(nowShape.dotList[-1], lineDistance, SCREEN_SIZE, screenCoordinate), BLACK)#마지막 하나 안그려지므로.(len-1)이니까
    elif mode == 'MakingCircle':
        if nowShape.centerDot != -1:
            drawDot(screen, dotToScreenDot(nowShape.centerDot, lineDistance, SCREEN_SIZE, screenCoordinate), BLACK)
            if nowShape.radius != -1:
                pygame.draw.circle(screen, BLACK, dotToScreenDot(nowShape.centerDot, lineDistance, SCREEN_SIZE, screenCoordinate), round(nowShape.radius*lineDistance), 1)
    for shape in shapeList:
        if type(shape) is Poly:
            for i in range(len(shape.dotList)-1):
                pygame.draw.line(screen, BLACK, dotToScreenDot(shape.dotList[i], lineDistance, SCREEN_SIZE, screenCoordinate), dotToScreenDot(shape.dotList[i+1], lineDistance, SCREEN_SIZE, screenCoordinate))
                drawDot(screen, dotToScreenDot(shape.dotList[i], lineDistance, SCREEN_SIZE, screenCoordinate), BLACK)
            drawDot(screen, dotToScreenDot(shape.dotList[-1], lineDistance, SCREEN_SIZE, screenCoordinate), BLACK)
        else:
            drawDot(screen, dotToScreenDot(shape.centerDot, lineDistance, SCREEN_SIZE, screenCoordinate), BLACK)
            pygame.draw.circle(screen, BLACK, dotToScreenDot(shape.centerDot, lineDistance, SCREEN_SIZE, screenCoordinate), round(shape.radius*lineDistance), 1)
